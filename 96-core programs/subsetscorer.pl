...
{chomp $eachtag;
	...
	if(length $clocation>0){$clshort=substr($clocation,0,((length $clocation)-2));print $out "$clshort\t$tagpos[1]";}else{print $out "$tagpos[0]\t-\t-\t-\t$tagpos[1]";}
        system("LC_ALL=C grep -w $eachtag $ARGV[0]/hpcjcl/snpreport.srt >$ARGV[0]/hpcjcl/tmplist.$ARGV[1]");
        foreach $a (@lines)
        {
                chomp $a;
                @call=`LC_ALL=C grep -w $a $ARGV[0]/hpcjcl/tmplist.$ARGV[1]`;
        $calls=scalar @call;
        if($calls == 0){print $out "\t-";}else{@calla=split /\t/,$call[0];chop $calla[4];print $out "\t$calla[4]";}
        }
print $out "\n";
}
